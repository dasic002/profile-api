...
# user can retrieve a post with valid id
# user cannot retrieve a post with an invalid id
# user can update the posts they own
# users cannot update the posts they do not own
class PostDetailViewTests(APITestCase):
    ...
    def test_can_retrieve_a_post_with_valid_id(self):
        response = self.client.get('/posts/1/')
        self.assertEqual(response.data['title'], 'a title')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_cannot_retrieve_a_post_with_an_invalid_id(self):
